...
void modifyArr(int *arr, int &size, int n, int k)
{
      ...
      for (int j = k - 1; j < size - n; j++)
      { // цикл от j = k - 1 до size - n, где K - элемент массива с которого надо начинать удаление элементов, n - количество элементов котороые надо удалить,
...
void modifyArr(int *arr, int *be, int &size, int N, int K)
{
      ...
      for (int i = K + N - 1; i < size; i++)
      {                                 // от i = K + N - 1, до size(размер массива arr)
